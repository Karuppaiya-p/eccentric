...
<form name="addform" action="<?php echo $_SERVER["REQUEST_URI"]?>" method="post" enctype="multipart/form-data" novalidate>
	<table id="file">
	<?php
	$work=$conn->query("Select * from work where leader_approved=1 and leader_by='".$_SESSION["username"]."'");
	if($work->num_rows>0)
	{
		$i=1;
		while($row=$work->fetch_assoc())
		{
			 $query2="SELECT GROUP_CONCAT(DISTINCT username ORDER BY username ASC SEPARATOR ',') as users FROM view_by where f_work=".$row["id"]." and group_name='".$_SESSION["group_name"]."'" ;
			...
			echo '	<tr style="background-color:#999">
					...
					<tr>
						...
						<td><textarea name="leader_comments'.$row["id"].'">'.$row["leader_comments"].'</textarea></td>
					</tr>
					<tr>
						<td>Update</td>
						<td><button name="update" value="'.$row["id"].'" type="submit">Update</textarea></td>
					</tr>
				</tr>';
				$i++;
		}
